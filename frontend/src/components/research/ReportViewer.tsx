...
function generateBibTeX(citations: Citation[], task: Task): string {
  const date = new Date().getFullYear()
  const cleanTitle = task.question.replace(/[{}]/g, '')

  let bib = `% BibTeX export generated by Valyu Scientific Research Platform\n`
  bib += `% Date: ${new Date().toISOString().slice(0, 10)}\n\n`

  bib += `@techreport{valyu_synthesis_${task.id.slice(0, 8)},\n`
  bib += `  title = {${cleanTitle}},\n`
  bib += `  author = {{Valyu Research Synthesis Engine}},\n`
  bib += `  year = {${date}},\n`
  bib += `  institution = {Valyu Scientific Discovery Platform},\n`
  bib += `  note = {Automated literature review and evidence synthesis}\n`
  bib += `}\n\n`

  citations.forEach((c) => {
    const key = `source_${c.number}`
    const safeTitle = (c.title || 'Untitled Source').replace(/[{}]/g, '')
    const domain = safeHostname(c.url)
    bib += `@misc{${key},\n`
    bib += `  title = {${safeTitle}},\n`
    bib += `  url = {${c.url}},\n`
    bib += `  howpublished = {\\url{${c.url}}},\n`
    bib += `  note = {Source [${c.number}] accessed via ${domain}}\n`
    bib += `}\n\n`
  })
...
function generateLaTeX(task: Task, citations: Citation[]): string {
  const cleanTitle = task.question.replace(/[#&%$_{}~^\\]/g, '\\$&')
  const body = (task.report ?? '')
    .replace(/^##\s+(.+)$/gm, '\\section{$1}')
    .replace(/^###\s+(.+)$/gm, '\\subsection{$1}')
    .replace(/^####\s+(.+)$/gm, '\\subsubsection{$1}')
    .replace(/\[(\d+)\]/g, '\\cite{source_$1}')
  ...
  tex += `\\title{\\textbf{${cleanTitle}}}\n`
  tex += `\\author{Valyu Scientific Synthesis Engine}\n`
  tex += `\\date{\\today}\n\n`
  tex += `\\begin{document}\n`
  tex += `\\maketitle\n\n`
  tex += body + `\n\n`
  ...
  tex += `\\begin{enumerate}\n`
  citations.forEach((c) => {
    tex += `  \\item \\label{source_${c.number}} ${c.title.replace(/[#&%$_{}~^\\]/g, '')} \\newline \\url{${c.url}}\n`
  })
  tex += `\\end{enumerate}\n\n`
  tex += `\\end{document}\n`
...
export function ReportViewer({ task, onPodcastRequested }: ReportViewerProps) {
  ...
  const handleDownload = (format: 'md' | 'bib' | 'tex') => {
    if (!task.report) return
    setExportOpen(false)
    ...
    if (format === 'md') {
      ...
      content = generateBibTeX(citations, task)
      mime = 'application/x-bibtex;charset=utf-8'
      ext = 'bib'
    } else if (format === 'tex') {
      content = generateLaTeX(task, citations)
    ...
    a.href = url
    a.download = `valyu-report-${task.id.slice(0, 8)}.${ext}`
    a.click()
    ...
    <div className="flex h-[calc(100vh-56px)] overflow-hidden">
      {headings.length > 0 && tocOpen && (
        <>
          ...
          <aside
            className="fixed md:static inset-y-0 left-0 z-50 md:z-auto flex flex-col w-72 md:w-64 flex-shrink-0 border-r overflow-hidden theme-surface animate-in slide-in-from-left-4 duration-200 no-print"
              ...
              {headings.map((h) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'w-full text-left text-xs py-1.5 px-2.5 rounded-lg transition-all line-clamp-2 cursor-pointer',
                      h.level === 3 ? 'pl-5 text-[11px]' : 'font-medium',
                      isActive
                        ? 'bg-purple-500/15 text-purple-500 font-semibold border-l-2 border-purple-500 pl-2'
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border border-purple-500/30 bg-purple-500/10 text-purple-500 hover:bg-purple-500/20 transition-all cursor-pointer"
              >
              ...
              {exportOpen && (
                <>
                  ...
                  <div
                    className="absolute right-0 top-full mt-1.5 w-56 rounded-xl p-1.5 z-50 shadow-2xl border theme-surface"
                    style={{
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs hover:bg-purple-500/15 transition-colors text-left"
                      style={{ color: 'var(--text-1)' }}
                    >
                      <FileText className="w-3.5 h-3.5 text-blue-400" />
                      <div>
                        <div className="font-medium">Markdown (.md)</div>
                        <div className="text-[10px]" style={{ color: 'var(--text-3)' }}>Full text with math & badges</div>
                      </div>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-[10px]" style={{ color: 'var(--text-3)' }}>For Zotero, Mendeley & Overleaf</div>
                      </div>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-[10px]" style={{ color: 'var(--text-3)' }}>Ready-to-compile academic template</div>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-[10px]" style={{ color: 'var(--text-3)' }}>Clean paper layout via browser</div>
                      </div>
                    </button>
                  </div>
                </>
          ...
          {citations.length > 0 && (
            <div className="max-w-3xl mx-auto mt-12 lg:hidden no-print">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => handleDownload('bib')}
                  className="flex items-center gap-1 text-[11px] text-purple-400 font-medium px-2 py-1 rounded bg-purple-500/10"
                >
          ...
          >
            {citOpen && (
              <div className="flex items-center justify-between w-full pr-2">
                ...
                <button
                  onClick={() => handleDownload('bib')}
                  className="text-[10px] flex items-center gap-1 px-1.5 py-0.5 rounded font-medium transition-colors bg-purple-500/10 text-purple-400 hover:bg-purple-500/20"
                  title="Export all citations as BibTeX file"
                >
